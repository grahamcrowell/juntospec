...
| 6 | `file-patterns.md` | Backlog management, LLM-optimized patterns, project structure, document filing rule. | Backlog guidelines (target <10KB), state-path resolution preamble naming both key families (state + category; the state family includes the review-history key the change-review command files to), the **filing rule** (classify a produced document by type -> resolve that type's key -> file at the owning node -> fall back to the artifacts root when the layout has no per-type surface; plus "a review is not a design", which names the RESOLVED review-history location rather than a literal path, and "a fact needs provenance"), `.claude/` structure (minimum viable + mature with state/artifacts/archive), header/detail pattern (index <5KB + detail/YYYY-QN.md, size thresholds: <10KB single / 10-25KB split-consider / >25KB must-split) | Structural guidance for setup and refactoring, not execution; header/detail pattern is reusable solution to growing-archive bloat; the filing rule is the single source that the artifact-producing commands cross-reference rather than duplicate, so a produced document gets filed rather than dumped |
