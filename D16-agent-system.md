...
**Default model class and cognitive tier per role**: Each role above has a default model class (authoring / advisory) and a default effort tier ({tier-routine} / {tier-implementation} / {tier-reasoning}) defined in `[model-and-effort-selection-for-spawned-agents](D32-execution-models.md#immutable-6-model-and-effort-selection-for-spawned-agents)`. The class is keyed by what the role produces - authoring where it writes code or a durable artifact, advisory where it reads and forms a view - not by the role's seniority or the gravity of its domain. The tier selects depth of reasoning within that class. The manager applies the function-first selection rules from that section when spawning - reviewer-slot and Complex-tier-lead spawns override both the per-role default tier and, where the rule lands above the role's class, the class itself.
