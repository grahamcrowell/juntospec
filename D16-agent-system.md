...
**Default cognitive tier per role**: Each role above has a default effort tier ({tier-routine} / {tier-implementation} / {tier-reasoning}) defined in `[model-and-effort-selection-for-spawned-agents](D32-execution-models.md#immutable-6-model-and-effort-selection-for-spawned-agents)`. Every role runs on the same model; the tier selects depth of reasoning, not capability. The manager applies the function-first selection rules from that section when spawning - reviewer-slot and Complex-tier-lead spawns override the per-role default.
